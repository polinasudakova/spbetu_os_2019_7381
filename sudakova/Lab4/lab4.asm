...
DATA SEGMENT
	...
	resUnload db "Resident was restored", 0dh, 0ah, '$'
	resAlrSet db "Resident is already loaded", 0dh, 0ah, '$'
	resLoad db "Resident is loaded", 0dh, 0ah, '$'
	
	ourPSP dw 0	
	
		;------------------DATA-------------------
	KEEP_SP dw ? ;Выделение памяти под хранение регистра SP
	KEEP_SS DW ? ;Выделение памяти под хранение рагистра SS
...
start:
	...
	mov ss, dx              ; Помещаем смещение до выделенной памяи под стек
...
not_load_block:
	...
	call RES_LOAD           ; Загрузка резидента.
...
unload_block:
	...
	call RES_UNLOAD; Выгрузка резидента
